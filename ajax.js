...
let listOfMatches = {};
let badgesDictionary = new Map();
var chart;

async function getPremierLeagueFixtures() {
    const response = await axios.get('https://api.sportsdata.io/v3/soccer/scores/json/GamesByCompetition/england-premier-league?key=eb75a56f6bf04c73b38127418f627e26&season=current');
...
class Summary {
    constructor(nome) {
      ...
      this.g4s = 0;
      this.z4s = 0;
      this.histograma = undefined;//new Array(20).fill(0);
    }
    compareTo(other) {
...
function displaySummary(summary){
    ...
    summary.forEach((stats, i) => {
        ...
        table += `<tr><td>${i+1}</td>
                    ...
                    <td>${(100*stats.histograma.slice(0,6).reduce((a,b) => a+b,0)/N_SIMS).toFixed(2)}</td>
                    <td>${(100*stats.histograma.slice(6,12).reduce((a,b) => a+b,0)/N_SIMS).toFixed(2)}</td>
                    <td>${(100*stats.histograma.slice(12,16).reduce((a,b) => a+b,0)/N_SIMS).toFixed(2)}</td>
...
const runSimulation = alistOfMatches => {
    ...
    for (let i = 0; i < N_SIMS; i++) {
        ...
        for (let pos = 0; pos < classif.length; pos++) {
            if(stats.get(classif[pos].name).histograma === undefined) {
              stats.get(classif[pos].name).histograma  = new Array(classif.length).fill(0);
            }
...
function getMatchListFromServer(){
    const urlParams = new URLSearchParams(window.location.search);
    const myParam = urlParams.get('camp');
    console.log(myParam);
}
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    //{   
    //    ready = true;
    //    var eventSource = new EventSource("ajax.php");
    //    eventSource.onmessage = function(event) {
    //       var data = JSON.parse(event.data);
    //        document.getElementsByClassName("progress-bar")[0].style.width = data.progress;
